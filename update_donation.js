document.getElementById('updateDonationForm').addEventListener('submit', async (event) => {
    ...
    try {
        ...
        if (response.ok) {
            document.getElementById('message').textContent = 'Donation updated successfully!';
        } else {
            document.getElementById('message').textContent = 'Failed to update donation.';
        }
    } catch (error) {
